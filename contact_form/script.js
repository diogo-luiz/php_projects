...
form.onsubmit = (e)=>{
    ...
    xhr.onload = ()=>{
        if(xhr.readyState == 4 && xhr.status == 200){// if ajax response status is 200 & ready status is 4 means there is no any error
            ...
            if(response.indexOf("Área de email e mensagem são necessários!") != -1
            || response.indexOf("Insira um email válido!")
            || response.indexOf("Desculpe, houve um problema ao enviar seu email!")){
